...
class KDTree
{
  ...
  private:
     ...
     */void copy(const KDTree<Dim> &rhs);
     int goPartition(int a, int b, int count, vector<Point<Dim>>& newPoints);
     void buildTree(int a, int b, int count, KDTreeNode *& node, vector<Point<Dim>>& newPoints);
     Point<Dim> goQuickSelect(int a, int b, int k, int count, vector<Point<Dim>>& newPoints);
     void rebuild(KDTreeNode *cNode, KDTreeNode *node);
     double distance(const Point<Dim>& a, const Point<Dim>& b) const;
     void find_helper(KDTreeNode* subroot, const Point<Dim>& target, Point<Dim>& currentBest, int d) const;
};
